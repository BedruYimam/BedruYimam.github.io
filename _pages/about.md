---
layout: single
title: "About"
permalink: /about/
author_profile: true
classes: wide
---

<div style="text-align: center; margin-bottom: 3rem;">
  <img src="/images/bedru.jpg" alt="Bedru Yimam Ahmed" style="width: 250px; height: 250px; border-radius: 50%; border: 5px solid #2e8b57; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.15); margin-bottom: 1.5rem;">
  <h1 style="margin-bottom: 0.5rem; color: #2c3e50;">Bedru Yimam Ahmed</h1>
  <h3 style="color: #2e8b57; margin-top: 0; margin-bottom: 0.5rem;">Lecturer & Researcher in Information Technology</h3>
  <p style="font-size: 1.1rem; color: #555;">
    <i class="fas fa-university" style="color: #2e8b57;"></i> Wollo University, Dessie, Ethiopia<br>
    <i class="fas fa-envelope" style="color: #2e8b57;"></i> bedruy4@gmail.com
  </p>
</div>

## Biography

I am a dedicated Information Technology lecturer and researcher at Wollo University in Ethiopia, specializing in Artificial Intelligence, Machine Learning, and Computer Vision. My academic journey is driven by a passion for leveraging technology to address local and regional development challenges while contributing to the advancement of computer science education in Ethiopia.

With a strong commitment to both teaching and research, I strive to bridge the gap between theoretical knowledge and practical applications, preparing the next generation of Ethiopian technologists and researchers to tackle complex problems through innovative solutions.

## Education



**Master of Science (MSc) in Information Technology**  
*[University Name], [City, Country]*  
*[Year] - [Year]*  
*Thesis: [Your Thesis Title]*  
*Advisor: [Advisor Name]*

**Bachelor of Science (BSc) in Information Technology**  
*[University Name], [City, Country]*  
*[Year] - [Year]*

## Academic Positions

**Lecturer, Department of Information Technology**  
*Wollo University, Dessie, Ethiopia*  
*[Start Year] - Present*

**Responsibilities:**
- Teaching undergraduate and graduate courses in computer science
- Supervising student research projects and theses
- Developing curriculum and course materials
- Conducting research in AI and related fields
- Participating in departmental and university committees

**[Previous Position]**  
*[Institution Name], [Location]*  
*[Start Year] - [End Year]*

## Research Focus

My research program centers on several interconnected areas:

### 1. Artificial Intelligence for Social Good
Developing AI solutions that address specific challenges in the Ethiopian and African contexts, including:
- Agricultural technology for smallholder farmers
- Healthcare applications for resource-constrained settings
- Educational technology for enhanced learning outcomes

### 2. Machine Learning and Computer Vision
- Developing efficient algorithms for image and video analysis
- Exploring transfer learning techniques for limited data scenarios
- Applications in remote sensing and environmental monitoring

### 3. Natural Language Processing for Ethiopian Languages
- Developing computational resources for under-resourced languages
- Machine translation and text analysis tools
- Speech recognition and synthesis systems

### 4. Technology-Enhanced Education
- Developing adaptive learning systems
- Educational data mining and learning analytics
- Virtual and augmented reality applications for education

## Teaching Philosophy

I believe in creating an engaging, inclusive, and practical learning environment where students:
- Develop strong foundational knowledge in computer science principles
- Gain hands-on experience with current technologies and tools
- Learn to think critically and solve complex problems
- Understand the ethical implications and societal impacts of technology
- Are prepared for both academic research and industry careers

## Courses Taught

### Undergraduate Courses
- Introduction to Programming
- Data Structures and Algorithms
- Artificial Intelligence
- Machine Learning
- Computer Vision
- Database Systems
- Software Engineering

### Graduate Courses
- Advanced Machine Learning
- Deep Learning
- Research Methods in Computer Science
- Natural Language Processing
- Computer Vision Applications

## Professional Service

### University Service
- Member, Curriculum Development Committee
- Coordinator, [Committee/Program Name]
- Advisor, Computer Science Student Association

### Conference Activities
- Program Committee Member, [Conference Name]
- Reviewer for [Journal/Conference Names]
- Session Chair, [Conference Name]

### Professional Memberships
- Association for Computing Machinery (ACM)
- Institute of Electrical and Electronics Engineers (IEEE)
- Ethiopian Computer Professionals Association

## Awards and Honors

- [Award Name], [Organization], [Year]
- [Fellowship/Grant Name], [Funding Organization], [Year]
- [Teaching Excellence Award], [Institution], [Year]
- [Best Paper Award], [Conference], [Year]

## Personal Interests

Beyond academia, I am passionate about:
- **Open Source Software**: Contributing to and promoting open source projects
- **Mentoring**: Guiding young researchers and students
- **Community Engagement**: Working on technology literacy initiatives
- **Reading**: Science fiction, technology history, and educational literature
- **Outdoor Activities**: Hiking and exploring Ethiopia's natural beauty

## Contact

I welcome discussions about research collaborations, student supervision, or potential projects. Please feel free to reach out:

**Office Location:**  
Department of Computer Science  
Wollo University  
Dessie, Ethiopia

**Email:** [bedruy4@gmail.com](mailto:bedruy4@gmail.com)

**Social Profiles:**  
[GitHub](https://github.com/BedruYimam) | [Google Scholar](https://scholar.google.com/citations?user=YOUR_ID) | [LinkedIn](https://linkedin.com/in/yourprofile) | [ORCID](https://orcid.org/YOUR_ORCID)

---

*Last updated: January 2024*

<style>
/* Custom styles for about page */
.page__content h2 {
  color: #2e8b57;
  border-bottom: 2px solid #e8f5e8;
  padding-bottom: 0.5rem;
  margin-top: 2.5rem;
}

.page__content h3 {
  color: #3cb371;
  margin-top: 1.5rem;
}

.page__content ul, .page__content ol {
  padding-left: 1.5rem;
}

.page__content li {
  margin-bottom: 0.5rem;
}

.social-links {
  display: flex;
  justify-content: center;
  gap: 1.5rem;
  margin: 2rem 0;
}

.social-links a {
  font-size: 1.2rem;
  color: #2e8b57;
  text-decoration: none;
  transition: color 0.3s ease;
}

.social-links a:hover {
  color: #3cb371;
}

@media (max-width: 768px) {
  .social-links {
    flex-direction: column;
    align-items: center;
    gap: 1rem;
  }
}
</style>

<!-- Add Font Awesome Icons -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
<!-- Add Academicons for academic icons -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/jpswalsh/academicons@1/css/academicons.min.css">
